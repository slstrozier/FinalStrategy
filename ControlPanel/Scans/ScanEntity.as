package ControlPanel.Scans{
	...
	public class ScanEntity extends MovieClip{
		...
		private var databaseObj:Object;
		
		
		public function ScanEntity(isCompared:Boolean, databaseObject:Object) {
			databaseObj = databaseObject
			STRATEGY = Strategy.getInstance();
			isComparedEntity = isCompared;
			textDescription = new TextField();
			infoLabel = new TextField  ;
			_TradableEntity = "";
			_Treatment = "";
			_Database = databaseObject.DataName;
			_HumanDatabase = databaseObject.HumanName;
			_MathOperator = "";
		...
		function createSummery():String{
			...
			summery =  _HumanDatabase + "'s " + type +  " with " + _HumanTreatment;
			
			if(isCandle){
				summery = "Find a Tradable in " + _HumanDatabase  + " who's Candle Stick Pattern " + _candleOperator;
			}
			return summery;
		}
		...
		function treatmentChange(event:Event):void
		{
			...
			retrieveFields(_Database, _Treatment);
		...
		*/
		...
		parameterDescriptions:Array, object:Object, parameterString:String, defaults:Array = null):void
		{
			...
			if(getChildByName("Entity")){
			   ...
			   dispatch("comparedScanEntityRemoved", new Array);
			   }
			...
			for (var i:int = 0; i < Number(numFields); i++)
			{
				...
				if(parameterDescriptions[i] == "ENTITY"){
					...
					dispatch("comparedScanEntityAdded", new Array);
				}
				if(parameterDescriptions[i] != "ENTITY") {
		...
		function retrieveTreatments(database:String){
			var urlDriver:URLFactory = new URLFactory("http://backend.chickenkiller.com:8080/sample/ServiceFlash?TASK=TREATSFORDB&DBS=" + database);
		...
		function retrieveFields(database:String, treatment:String):void{
			var urlDriver:URLFactory = new URLFactory("http://backend.chickenkiller.com:8080/sample/ServiceFlash?TASK=DATAFIELDS_SCAN&DBS="  + database + "&TREAT=" + treatment);
