package ControlPanel.QSB
{
	...
	public class Indicator extends MovieClip
	{
		...
		function setQueryString():String
		{
			...
			var condEntity = "TYPE=IND&ENTITY=" + _Indicator + "(" + iPara + ")" + "[" + _Treatment + tPara + "]{" + _Database + "}&START=" + this.startDateFull + "&END=" + this.endDateFull;
			return condEntity;
		}
		...
		function startUp():void
		{
			...
			createDataBox("Indicators", -200, 115, "Select an Indicator" ,STRATEGY.GetIndicators(), indicatorParameters = new Array(), indicatorComboBox, indicatorParameters_String, _Indicator);

			indicatorComboBox.dropdown.addEventListener(Event.CHANGE, activateMathOps);
			indicatorComboBox.width = 225;
			indicatorComboBox.dropdownWidth = 225;

		}
		function activateMathOps(event:Event):void
		{
		...
		function addTreatment(event:Event):void
		{
			...
			treatmentComboBox = new ComboBox();
			createDataBox("Treatments", -65, 240, "Select Treatment", STRATEGY.GetTreatments(), treatmentsParameter = new Array(), treatmentComboBox, treatmentsParameter_String,_Treatment );
